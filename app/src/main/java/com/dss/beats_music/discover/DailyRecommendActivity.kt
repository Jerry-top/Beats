package com.dss.beats_music.discover
...
import com.dss.beats_music.discover.viewmodel.DailyRecommendViewModel
...
//        songList = Gson().fromJson<List<Song>>(data,object :TypeToken<List<Song>>(){}.type)
        ...
        bar.viewTreeObserver.addOnGlobalLayoutListener {
            ...
            showView.viewTreeObserver.addOnGlobalLayoutListener {
                consecutiveLayout.setOnVerticalScrollChangeListener { v, scrollY, oldScrollY, scrollState ->
                    barBackgroundView.alpha =
                        (scrollY * 1.0 / (showView.height - bar.height)).toFloat()
                }
            }
     ...
     */
    private fun convert(
        source: Bitmap, screenWidth: Int,
        barHeight: Int, showHeight: Int, bottomHeight: Int
    ): Array<Bitmap> {
...
//        Log.e("tag","width = ${width},height=${height},scale = ${scale}")
        ...
        // 从scaleBitmap截取三张图片
        var barBitmap =
            Bitmap.createBitmap(scaleBitmap, 0, showHeight - barHeight, screenWidth, barHeight)
        var showBitmap = Bitmap.createBitmap(scaleBitmap, 0, 0, screenWidth, showHeight)
        var bottomBitmap =
            Bitmap.createBitmap(scaleBitmap, 0, showHeight, screenWidth, bottomHeight)
        scaleBitmap.recycle()
